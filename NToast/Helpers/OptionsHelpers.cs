...
namespace NToastNotify.Helpers
{
    internal class OptionsHelpers
    {
        ...
        public static void EnsureSameType<T>(object obj) where T : new()
        {
            ...
            {
                throw new InvalidCastException("Wrong options type passed. "
                    + "Make sure you are passing the right toast options types. "
                    + $"Passed options type {obj.GetType().Name}. Expected options type {typeof(T).Name}"
                );
            }
        }
